...
async function proxy(request?:Request){
  ...
  try{
    ...
    const response=await fetch(new URL('/workspace',url),{method:request?'PUT':'GET',redirect:'error',headers:{Authorization:`Bearer ${env.ACCOUNT_SERVICE_SECRET}`,'X-Contribly-User-Id':user.userId,'Content-Type':'application/json'},body,signal:AbortSignal.timeout(15000)});
